...
class TcGerminTransfer extends Model
{
    use HasFactory;
    use SoftDeletes;
    protected $guarded = [];
    ...
    {
        $toBack = $data->to_self;
        $toNext = $data->to_rooting;
        $transId = $data->id;
        $allowBack = $allowNext = true;

        if($toBack != 0){
            $bottleId = array_column(TcGerminBottle::select('id')->where('tc_germin_transfer_id',$transId)->get()->toArray(),'id');
        ...
        if($toNext != 0){
            $bottleId = array_column(TcGerminBottle::select('id')->where('tc_germin_transfer_id',$transId)->get()->toArray(),'id');
            $q = TcGerminObDetail::whereIn('tc_rooting_bottle_id',$bottleId)->get()->count();
