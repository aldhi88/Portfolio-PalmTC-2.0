...
class TcLiquidTransfer extends Model
{
    use HasFactory;
    use SoftDeletes;
    protected $guarded = [];
    ...
    {
        $toBack = $data->to_self;
        $toNext = $data->to_matur;
        $transId = $data->id;
        $allowBack = $allowNext = true;

        if($toBack != 0){
            $bottleId = array_column(TcLiquidBottle::select('id')->where('tc_liquid_transfer_id',$transId)->get()->toArray(),'id');
            $q = TcLiquidObDetail::whereIn('tc_liquid_bottle_id',$bottleId)->get()->count();
            $allowBack = $q==0?true:false;
        }

        if($toNext != 0){
            $bottleId = array_column(TcMaturBottle::select('id')->where('tc_liquid_transfer_id',$transId)->get()->toArray(),'id');
