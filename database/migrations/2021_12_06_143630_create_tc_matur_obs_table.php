...
class CreateTcMaturObsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('tc_matur_obs', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('tc_init_id');
            $table->bigInteger('tc_worker_id')->nullable();
            $table->char('alpha',2)->nullable();
            $table->smallInteger('total_bottle_matur')->default(0);
    ...
    {
        Schema::dropIfExists('tc_matur_obs');
    }
}
