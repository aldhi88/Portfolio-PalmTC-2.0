<form id="editStep1">@csrf
    ...
    <div class="row">
        ...
        <div class="col">
            <div class="form-group">
                <label class="font-weight-bold">Sub Culture</label>
                <select name="sub" class="form-control form-control-sm">
                    @foreach ($data['subCultere'] as $item)
                        <option {{ $item->sub == $dtSession['sub']?'selected':null }} value="{{ $item->sub }}">{{ $item->sub }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
...
@include('modules.embryo_transfer.component.include.step1_edit_js')