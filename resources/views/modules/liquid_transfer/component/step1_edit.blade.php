<form id="editStep1">@csrf
    ...
    <div class="row">
        ...
        <div class="col">
            <div class="form-group">
                <label class="font-weight-bold">Alpha/Cycle</label>
                <select name="alphaCycle" class="form-control form-control-sm">
                    @foreach ($data['subCultere'] as $item)
                        <option {{ $item->alpha == $dtSession['alpha']?'selected':null }} value="{{ $item->alpha.'/'.$item->cycle }}">{{ $item->alpha.'/'.$item->cycle }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
...
@include('modules.liquid_transfer.component.include.step1_edit_js')