<form id="createStep1">@csrf
    ...
    <div class="row">
        ...
        <div class="col">
            <div class="form-group">
                <label class="font-weight-bold">Alphabetic/Cycle</label>
                <select name="alphaCycle" class="form-control form-control-sm">
                    @foreach ($data['subCultere'] as $item)
                        <option value="{{ $item->alpha.'/'.$item->cycle }}">{{ $item->alpha.'/'.$item->cycle }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
...
@include('modules.liquid_transfer.component.include.step1_create_js')